...
public class TimingUtil {
  public static <S extends IState<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(
      ...
      double step_size,
      final List<ITimingConstraint<S>> constraints,
  ...
  public static <S extends IState<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(
    ...
    for (int i = 0; i < states.size(); ++i) {
      ...
      while (true) {
        ...
        for (final ITimingConstraint<S> constraint : constraints) {
          constraint_state.max_velocity =
              Math.min(
        ...
        for (final ITimingConstraint<S> constraint : constraints) {
          final ITimingConstraint.MinMaxAcceleration min_max_accel =
        ...
        // TODO: Simply using the new max acceleration is guaranteed to be valid, but
    ...
    for (int i = states.size() - 1; i >= 0; --i) {
      ...
      while (true) {
        ...
        // TODO: Simply using the new min acceleration is guaranteed to be valid, but
        // may be too conservative.
        // Doing a search would be better.
        final double actual_acceleration =
            (constraint_state.max_velocity * constraint_state.max_velocity
    ...
    for (int i = 0; i < states.size(); ++i) {
      ...
      if ((Double.isNaN(accel) || Math.abs(accel) <= kEpsilon)
          && (Double.isNaN(v) || Math.abs(v) <= kEpsilon)
          && i > 0) {
      ...
      // System.out.println("constrained_state" + constrained_state.state.toString());
      timed_states.add(
  ...
  protected static class ConstrainedState<S extends IState<S>> {
    public S state;
